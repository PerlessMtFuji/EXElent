...
def test_project_in_home_prefers_desktop_over_home(tmp_path, monkeypatch):
    """A project directly in the home directory should not create output
    in ~/, because a non-technical user does not look there manually and
    will not find the EXE file."""
    home = tmp_path / "dom"
...
def test_downloads_is_a_fallback_when_desktop_missing(tmp_path, monkeypatch):
    """When Desktop does not exist, Downloads is better than the home directory."""
    home = tmp_path / "dom"
    root = _make(home / "projekt", {"main.py": "print(1)"})
...
# --- B08: accepted file inventory ---


def test_source_inventory_contains_py_files_with_hashes(tmp_path):
    """B08: the plan contains an inventory with hashes of Python files."""
...
def test_source_inventory_contains_data_files(tmp_path):
    """B08: data files (JSON, CSV, etc.) are also in the inventory."""
...
def test_source_inventory_is_sorted_by_path(tmp_path):
    """B08: inventory is sorted by path — deterministic output."""
...
def test_source_inventory_excludes_converted_files(tmp_path):
    """B08: files generated by TXT→PY conversion do not exist on disk,
    so they have no inventory entry — their content lives in plan.converted."""
    root = _make(tmp_path / "p", {"main.py": "import kod", "kod.txt": "print(1)\n"})
    plan = make_plan(analyze_project(root))
    inv_paths = {e.rel_path for e in plan.source_inventory}
    # The original TXT should be in the inventory (for verification).
...
# --- B05: dependency origin ---


def test_dependency_origin_from_manifest(tmp_path):
    """B05: dependency from requirements.txt has origin='manifest'."""
    root = _make(tmp_path / "p", {"main.py": "import os"})
    (root / "requirements.txt").write_text("requests>=2.0\n", encoding="utf-8")
    plan = make_plan(analyze_project(root))
    dep = next(d for d in plan.packages if "requests" in d)
    # Origin propagates from the resolver — but packages is tuple[str],
    # so we verify through analysis.
    analysis = analyze_project(root)
...
def test_dependency_origin_from_import(tmp_path):
    """B05: dependency detected from import scan has origin='import'."""
    root = _make(tmp_path / "p", {"main.py": "import requests"})
    analysis = analyze_project(root)
